...
- before final review of a medium- or high-semantic-risk change, perform bounded semantic closure: identify the small set of concepts whose meaning, ownership, lifecycle, status, supported behavior, or authority changed, then search maintained current-state documentation, planning, tests, examples, interfaces, and executable configuration for the new/current vocabulary and plausible prior-state assumptions; reconcile stale behavior, ownership, and status claims in the same change, including planning sequence when it changed, without expanding into unrelated repository-wide consistency work;
