...
3. **Normalize a stale base before substantive review.**
   - For an ordinary PR, prefer the canonical merge-style Update branch synchronization path from `AGENTS.md`, normally `node infra/dev/pr-reconcile.mjs <pr-number>`; the helper uses GitHub's repository-scoped API and does not require a local checkout.
   - If that native operation is unavailable, a repository-scoped Git-data fallback is permitted only when the exact merge tree is mechanically provable from complete, non-truncated recursive tree snapshots and the branch can be advanced with `force=false`; recursively expand truncated trees or return the PR to the author. Use the detailed capability boundary in `AGENTS.md`, never semantic conflict resolution or a forced update.
   - For a research-evidence workspace carrying handed-off evidence coordinates, use either safe history-preserving route and verify the handed-off SHAs and pre-update head remain reachable.
   ...
   - If synchronization conflicts, requires semantic judgment, has an unsupported tree/identity shape, lacks permission, or cannot be performed safely in the current harness, stop and return the PR to its author/implementation owner. Do not file a reconciliation finding or post a disposition against the stale head merely to request a conflict-free sync.
