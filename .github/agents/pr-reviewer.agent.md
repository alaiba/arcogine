...
3. **Normalize a stale base before substantive review.**
   ...
   - If that native operation is unavailable, a repository-scoped Git-data fallback is permitted only when the exact merge tree is mechanically provable and the branch can be advanced with `force=false`; use the detailed capability boundary in `AGENTS.md`, never semantic conflict resolution or a forced update.
