...
- **Ordinary PR + no usable local checkout, but repository-scoped Git/GitHub data and mutation primitives are available:** use the connector fallback below when every invariant can be established mechanically.
...
6. Update the PR branch exactly once, directly from the inspected old head to the complete final head. Use a lease/compare-and-swap guard when the mutation surface provides one. If it does not, minimize the race by performing the required pre-update ref recheck immediately before the atomic ref update and then verify the result immediately afterward; do not use that weaker path when concurrent mutation risk cannot be acceptably controlled.
