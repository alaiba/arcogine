...
The **base-normalization protocol** is canonical; `infra/dev/pr-reconcile.mjs` is the preferred local adapter for ordinary PRs, not the protocol itself. Absence of a local checkout, `git`, or `gh` execution surface is therefore not by itself a reason to return a stale PR to its author. Route normalization by the capabilities and invariants of the current PR:

- **Ordinary PR + local checkout/git/gh available:** use `node infra/dev/pr-reconcile.mjs <pr-number>`.
- **Ordinary PR + no usable local checkout, but repository-scoped Git/GitHub data and mutation primitives including an atomic expected-head/lease/CAS update are available:** use the connector fallback below when every invariant can be established mechanically.
- **Research-evidence workspace carrying handed-off evidence coordinates:** use the history-preserving merge-style Update branch path described below; never rebase away handed-off SHAs.
...
Before returning an **ordinary** stale PR solely because `pr-reconcile.mjs` cannot execute locally, inventory the repository/GitHub mutation surfaces actually available in the harness and decide explicitly whether the connector fallback can satisfy every invariant below. Do not ask the author to run `pr-reconcile.mjs` merely because the reviewer lacks that particular adapter.

For the connector fallback, all of these conditions are mandatory:

1. Re-resolve the live base ref, current PR head, PR state, and head repository. Record the old head and merge base; require an open same-repository PR. The PR API's historical `base.sha` is not evidence that the base is current.
2. Establish that the rebase can be reproduced **mechanically**, without conflict resolution or semantic choice. A single-commit PR with no overlapping path changes between merge-base→live-base and merge-base→PR-head is the common simple case. For multiple commits, renames/deletes, mode changes, symlinks/submodules, overlapping paths, or other cases where exact replay cannot be proved from the available surfaces, do not flatten or guess; use a real Git rebase surface or return the PR to the author.
3. Construct the **complete final reconciled head before any branch mutation**. For the simple single-commit case, build the final tree on the live base using the PR's exact intended blobs and modes, then create one replacement commit whose parent is the live base. The final head must differ from the base and retain a non-empty intended PR diff. Never point the PR branch at the base commit as an intermediate step.
4. Verify the new commit obeys the repository's Git attribution rule before updating the branch: author and committer must identify the human repository owner, with no agent/bot attribution or prohibited trailers. If the available commit-creation primitive cannot establish and verify that identity, the fallback is unavailable.
5. Immediately before the one remote mutation, re-resolve both the live base and PR head. If either moved, discard the candidate and recompute from the new state.
6. Update the PR branch exactly once, directly from the inspected old head to the complete final head, and make that remote mutation conditional on the exact inspected old head through a lease, compare-and-swap, or equivalent atomic expected-head precondition. A pre-update ref recheck followed by an unconditional forced update is not sufficient. If the available mutation surface cannot enforce the expected old head atomically, the connector fallback is unavailable; use a real Git rebase surface or return the PR to the author/implementation owner.
7. After the update, verify the PR is still open, the live base is incorporated, `behind_by == 0`, `ahead_by > 0`, and the net live-base→new-head diff still represents the intended PR change. Re-resolve CI/checks and review only this normalized head.

This fallback is a mechanical implementation of the same safety contract as `pr-reconcile.mjs`; it is not permission to resolve conflicts, change intent, squash arbitrary multi-commit history, opportunistically clean up a branch, or bypass special evidence/Dependabot rules.
...
When an ordinary open PR is behind its base and a local checkout of that PR branch is available, reconcile it with `node infra/dev/pr-reconcile.mjs <pr-number>` by default. The helper reconciles against the live base branch ref, refuses to proceed if the live base or PR head moves during setup, aborts conflicts before remote mutation, constructs the complete rebased head first, updates the remote branch exactly once with a lease bound to the inspected old head, and verifies that the PR remains open and level afterward. When that local adapter is unavailable, use the connector fallback above rather than improvising or automatically returning the PR to its author.

A research-evidence workspace carrying handed-off report or adversarial-review coordinates is the other exception to the normal rebase path. Once an exact evidence `commit SHA + path` has been handed off under `docs/development/researching.md` §10, those commits must remain reachable by the same SHA while the workspace advances. If an open reconciliation PR from that workspace falls behind its base, use GitHub's history-preserving Update branch operation — `gh pr update-branch <pr-number>` **without** `--rebase`, or the platform-equivalent merge-style action when `gh` is unavailable — instead of `pr-reconcile.mjs`, raw rebase, or force-push. Re-resolve lifecycle afterward and verify that the pre-update PR head remains an ancestor of the new head and that the live base is incorporated. The same rule applies when the reviewer performs pre-review normalization. Do not require a separate branch merely to preserve those evidence coordinates.
