...
fn validate_scenario(config: &ScenarioConfig) -> Result<(), SimError> {
    ...
    if let Some(econ) = &config.economy {
        ...
        if !econ.initial_price.is_finite()
            ...
            || econ.initial_price > MAX_ECON_VALUE
        {
            return Err(SimError::OutOfRange {
        ...
        if !econ.base_demand.is_finite()
            || econ.base_demand < 0.0
            || econ.base_demand > MAX_ECON_VALUE
        {
            return Err(SimError::OutOfRange {
        ...
        if !econ.price_elasticity.is_finite()
            || econ.price_elasticity < 0.0
            || econ.price_elasticity > MAX_ECON_VALUE
        {
            return Err(SimError::OutOfRange {
        ...
        if !econ.lead_time_sensitivity.is_finite()
            || econ.lead_time_sensitivity < 0.0
            || econ.lead_time_sensitivity > MAX_ECON_VALUE
        {
            return Err(SimError::OutOfRange {
