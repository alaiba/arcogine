...
| `FactoryHandler` / `Job` (sim-factory) | **Aligned** | Owns machines/jobs/queues/status (production state). Each `Job` also *carries* its own `OrderPrice`, captured immutably at `OrderCreation`, and derives `orderValue()` — a commercial fact riding along with the production object, not something Factory conceptually owns. `CompletedSalesValue`/`completedSales` are private with accessors. No reference to `PricingState` — the factory never needs `OfferPrice`. `EventPayload.OrderCompleted`'s identifier field is named `jobId` (typed `JobId`), not `orderId` — there is no separate `Order`/`OrderId` concept yet, so naming the field honestly (rather than implying a distinction that doesn't exist in code) keeps a future `Order` split an explicit migration if one is ever warranted, not a silent one. `FactoryHandler.machines`/`.jobs` fields are package-private (were `public`); external callers get read-only `MachineView`/`JobView` via `machinesView()`/`jobsView()`/`job(JobId)` — `Job.start`/`completeStep` and `Machine.startJob`/`completeJob`/`setAvailability`/etc. are excluded from those interfaces, so nothing outside `sim-factory` can bypass event-driven mutation, mirroring the `Ledger`/`LedgerView` fix. `CompletedSalesValue`/`completedSales` are stored incrementally (a cached aggregate, not derived on read) — the invariant `CompletedSalesValue = Sum(orderValue(completed jobs))` and `completedSales = Count(completed jobs)` is documented in `docs/architecture.md` and locked down by `FactoryHandlerTest.completedSalesValueAndCountEqualTheSumAndCountOfCompletedJobs`, which independently recomputes both sides via `jobsView()`. |
| `FinanceHandler` / `Ledger` (sim-finance) | **Aligned** | Reacts only to `OrderCompleted`; posts balanced `JournalEntry` records under an immediate-settlement policy; `Posting`/`JournalEntry` construction rejects non-positive amounts and unbalanced entries, so an invalid entry cannot enter state. Module depends only on `sim-types`/`sim-core` — structurally cannot reach into `FactoryHandler`. The `double` → `BigDecimal` conversion at this boundary uses an explicit, named, tested policy (`CurrencyPolicy`: scale 2, `HALF_UP`), not an ad hoc `setScale(...)` call — so two independent conversions of the same quantity can't silently round differently. `Ledger.post` isn't reachable from outside `FinanceHandler` — `FinanceHandler.ledger()` returns the read-only `LedgerView`, not the concrete `Ledger`. `FinanceHandler` deliberately does not de-duplicate `OrderCompleted` deliveries (documented in its class Javadoc and locked down by `FinanceHandlerTest.deliveringTheSameOrderCompletedEventTwicePostsTwice`) — nothing today can actually redeliver an event, so a guard would defend against a scenario that can't occur; the concrete trigger for adding one is a future event-replay feature that replays into a *live* handler rather than a fresh one. The posted, quantized `BigDecimal` amount is documented as the financially authoritative figure for a transaction (distinct from, though expected to agree with, the originating `double` `OrderValue`) — see `docs/architecture.md`'s "The ledger amount is authoritative" note. `Sales` is documented as a model-specific account name for this simplified immediate-settlement policy, not a claim of standards-compliant revenue recognition — see the same section's `Sales` account note. |
| Full-chain integration coverage (`OrderCreation` → `Factory` → `OrderCompleted` → `Finance` → `Ledger`) | **Aligned** | `OrderLifecycleIntegrationTest` drives a single order through the real, wired `IntegratedHandler` (not hand-built events in isolation) and asserts no posting exists before completion, exactly one after, and the posted amount matches; a second test injects two `OrderCompleted`s at the same tick and confirms each posts its own entry (exercising the `Scheduler` FIFO fix in combination with Finance); a third runs a full multi-order scenario and asserts the count of `OrderCompleted` events in the log equals both `FactoryHandler.completedSales()` and `Ledger.entries().size()` — no completion is silently un-posted or double-posted. `FinanceScenarioIntegrationTest` covers the three conditions that actually stress the `OfferPrice`/`OrderPrice` invariant, independently recomputing the expected ledger total from each completed job's own captured price (not just checking agreement with Factory's own same-formula total): a manual `PriceChange` while two orders sit in backlog; a genuinely overloaded factory (nonzero backlog at run end) still accounting exactly for what completed; and a live agent making multiple `OfferPrice` interventions across a run, with the ledger total still matching each order's own creation-time price. |
| `IntegratedHandler` (sim-api) | **Aligned** | Explicit, fixed dispatch order (Pricing → Demand → Factory → Finance → Agent); observation construction is extracted to `AgentObservationProjector` rather than inlined. `IntegratedHandlerDispatchOrderTest` locks the sequence down as an executable fact — recording subclasses of each sub-handler append their name to a shared list on `handleEvent`, proving the actual call order, not just asserting a hand-written list. The order has no observable effect on any single event today (each event type is handled by exactly one sub-handler; the rest no-op), but a future accidental reorder now fails a test immediately rather than surfacing later as a subtle behavioral change once cross-handler effects within one dispatch become possible. `IntegratedHandler` itself owns `AgentEnabledChanged`, not a domain handler — `docs/architecture.md`'s "Event taxonomy" section classifies this as an orchestration/control event (which decision sources are active) rather than a domain event (a fact about simulated state), which is why it's handled at the orchestrator level. |
| `HandlerFactory` (sim-api) | **Aligned** | Pure wiring; no domain logic. |
| `AgentObservation` (sim-agents) | **Aligned** | Immutable, purpose-specific record; correctly limited to `OfferPrice`/backlog/lead-time/sales fields (never any order's `OrderPrice`). Fields named `offerPrice`/`completedSalesValue`, matching the resolved vocabulary. |
...
| `SnapshotBuilder` (sim-api) | **Aligned** | Legitimate, separate projection for the API/UI; `JobInfo.revenue` reads each job's own `orderValue()`, so it can't diverge from `FactoryHandler.completedSalesValue`. Does not expose Finance data (deliberate — no consumer needs it yet). The domain-observation-vs-API-DTO convention is now documented in `docs/architecture.md`'s "Domain observations vs. API/UI snapshots" section, with `FinanceObservation`/`SimSnapshot` as the worked example. |
| Module dependency structure | **Aligned; CI-enforced** | `ArchitectureTest` (sim-api, ArchUnit) checks: `sim-agents`/`sim-finance` never depend on `sim-factory`/`sim-economy`; `Ledger.post` is never called from outside `sim-finance`; `Job.start`/`completeStep` and `Machine`'s mutators are never called from outside `sim-factory`. A regression on any of these now fails the build, not just review. |

## Remaining migration plan

There is currently no P2/P3 backlog. (All P0 semantic-correctness and P1 structural items are resolved — see the matrix above. The state-query-interface idea previously listed here was reconciled with the "supplier vs. interface" rule below and demoted to a trigger-based future note, not scheduled work — see the next section.)

### Future note: state-query interfaces are trigger-based, not scheduled

`DemandModel`'s `DoubleSupplier`-bound reads of `OfferPrice`/lead time are working, sufficient, and should not be replaced merely to conform to a hypothetical pattern. `docs/architecture.md`'s "Query dependencies between domains" section already gives the rule: a scalar read stays a supplier; a multi-field or semantically-correlated read contract earns a purpose-specific interface (analogous to `AgentObservation`/`FinanceObservation`). There is nothing to do here until a real domain's read surface actually needs two or more correlated suppliers to represent one coherent fact — at that point, introduce the narrow interface for that domain specifically, rather than pre-building a generic `PriceSource`/`LeadTimeSource`-style pattern for domains that don't exist yet.
...
This works today because there is exactly one trusted decision source per concern. It stops working cleanly once that assumption breaks. The concrete trigger, sharpened to a decision rule rather than a vague "maybe later": **split commands from facts when a requested action can fail, be rejected, conflict with another request, or come from more than one authority.** None of that is true today — every `EventPayload`, once scheduled, is applied unconditionally, there is exactly one decision source per concern, and pre-validation happens before the event is emitted (e.g. `SalesAgent.decide()`'s `minPrice`/`maxPrice` clamp). Until one of those four conditions becomes true for some concern, scheduled events remain the simpler, sufficient mechanism — introducing the command/event split earlier would add ceremony without adding safety, since there would be nothing for the accept/reject step to actually decide. Examples of what would flip a given concern into "yes, split it": a future `FinanceAgent` and `SalesAgent` both able to affect price (conflict / multiple authorities); a UI command racing an agent decision (conflict); a domain rule that can legitimately reject a request rather than relying on the requester to have pre-validated it, like "can't take a machine offline mid-job" (can fail/be rejected). At that point, collapsing "requested" and "happened" into one concept stops being safe, and the two-step shape below becomes worth the extra ceremony:
...
- **No event-delivery idempotency guard in `FinanceHandler`** (or any other handler) **until event replay into a live handler is a real feature.** Nothing today can redeliver an event, so a duplicate-detection guard would defend against a scenario that can't occur, would hide a real dispatch bug rather than surface it if one were ever introduced elsewhere, and would grow unbounded state for the life of a run. `FinanceHandler`'s class Javadoc documents the trust assumption explicitly; `FinanceHandlerTest.deliveringTheSameOrderCompletedEventTwicePostsTwice` locks down that this is deliberate. If event replay is ever built, prefer a guard that throws on a genuine duplicate over one that silently skips — silent de-duplication would mask exactly the kind of dispatch bug this exists to catch.
...
- **No premature plugin framework** for future domains (inventory, procurement, workforce, maintenance). A state-query interface for `IntegratedHandler` should only be introduced once a real domain's read surface actually needs one (see "Future note: state-query interfaces are trigger-based, not scheduled" above) — not built ahead of that need.
...
- A new domain deserves this treatment when it owns mutable state with its own invariants and lifecycle — not merely because it has a new metric or helper function (see `docs/architecture.md`'s "When a new domain deserves its own module" section). A computed value over another domain's existing state, like `CompletedSalesValue`, belongs as a method on the existing owner, not a new module.
- Adding a genuine new domain means: (1) a new `XHandler implements EventHandler` that owns its own state, (2) a new `XObservation` record for whatever needs to read it, (3) one line in `IntegratedHandler`'s explicit sequence — not new setters scattered across existing handlers.
