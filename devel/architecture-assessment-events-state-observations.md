...
|---|---|---|---|
| P2 | Define a narrow state-query interface pattern for `IntegratedHandler` (e.g. `PriceSource`, `LeadTimeSource`) so adding a domain doesn't mean adding new direct field access from the orchestrator. Apply the "supplier vs. interface" rule now documented in `docs/architecture.md`'s "Query dependencies between domains" section — this is the point at which a domain's read surface would cross from scalar-supplier into interface territory. | Prepares for a third/fourth domain (inventory, procurement, workforce) without `IntegratedHandler` growing linearly with each new domain's internals. Prefer this over a generic event bus. | New narrow interfaces per domain; `IntegratedHandler` depends on interfaces, not concrete classes. |
