...
| Durable semantic fingerprint policy, controlled revision lineage, ChangeSets, requirements/assertions, conformance, evidence use, findings/exceptions | [Governance and conformance](architecture/governance-conformance.md) / [capability plan](planning/governance-conformance-capability.md) | Governance G1 identity/history, G2 semantic ChangeSet (initial slice), G3 requirement/assertion contract, and G4 conformance evaluation/findings (initial slice) are implemented; G5+ evidence-use/authorization capabilities remain outstanding; operational facts may be consumed as evidence, but Governance does not ingest telemetry or reconcile the twin |
...
1. **Fixtures are not sibling completion.** Governance G1 is complete, so new operational deployment/revision-bound reconciliation work should consume its authoritative revision identity/history rather than synthetic G1 revision fixtures. Governance G2's initial slice (semantic `ChangeSet`/`ImpactScope` and the factory-domain semantic comparator), Governance G3 (generic `Requirement`/`Assertion`/`RequirementCatalogue`), and Governance G4's initial slice (`ConformanceEvaluator`/`ConformanceEvaluation`/`Finding`) are also complete and available for consumption rather than synthetic substitution. Clearly scoped synthetic fixtures remain appropriate only for the still-outstanding G5+ evidence-use/authorization capabilities; likewise synthetic operational adapters do not satisfy Engine Readiness gates.
...
| [Engine Semantics v1](architecture/engine-semantics-v1.md) | Normative `engine-semantics:v1` specification — the complete result-affecting Engine interpretation, including existing dispatch/queue/decomposition rules and limits, plus the Gate 5 spatial transfer semantics |
...
| [ADR-0013: Execution context identity](architecture/decisions/0013-execution-context-identity.md) | Proposed O1 decision separating consequence classification from concrete operational-context identity, with opaque UUIDv4 identity, permanent kind binding, explicit establishment, checked context comparison, and no O1 context registry |
