...
The mechanical checker (`.github/scripts/check-durable-vocabulary.py`) recognizes high-confidence patterns (a `Gate` name followed by a number, a `REV-` prefix followed by digits, a `DH-` prefix followed by a letter) across every Markdown file under `docs/` outside `docs/planning/` (plus root `README.md`, this index, and `../product/interfaces/web/README.md`), and across Java/Javadoc comments, JS/TS/TSX and Python/Bash source and test comments, and GitHub Actions workflow definitions; a documented, narrow exemption list covers process/policy material (this file's own review/consistency guidance) where the coordinate syntax itself is the subject. Collision-prone compact forms (a single letter such as `G` or `O` followed by a digit) stay scoped to root `README.md`, this index, `docs/architecture/**`, `docs/product/**`, `docs/reference/**`, and `docs/examples/**`. PR review applies the broader semantic rule and catches context-dependent leakage outside what the checker can recognize safely. ADR-specific semantics-preserving editorial amendments follow the policy in [`architecture/decisions/README.md`](architecture/decisions/README.md).
