...
- **semantic field membership** — whether a given field is semantic or presentation, decided per field rather than assumed from "it's in the record." In particular: whether names (`ResourceDefinition.name()`, `OperationDefinition.name()`, etc. — currently hashed) are semantic content or a presentation label that shouldn't affect identity; and whether allocated IDs (`MachineId`, operation/product IDs — currently hashed) are semantic identity or surrogate references that a re-import with fresh IDs should be allowed to reproduce;
...
- **equality relationship** — what invariant, if any, is required between `FactoryModel.equals()` (today's generated Java record equality, itself structurally- and list-order-sensitive) and fingerprint equality: whether `FactoryModel.equals()` is intended to mean semantic equality, or merely representation equality that happens to coincide with it today.
...
A **controlled revision** is a persisted, controlled historical configuration state: it carries change-control provenance and lineage back to a prior revision. A controlled revision exists independently of approval or deployment — approval and deployment are separate lifecycle/evidence records that may reference the revision, not properties required for the revision to exist. This must represent both `candidate → approved revision → scheduled → deployed` and `approved revision → never deployed` without contradiction; a revision that is never approved, or approved but never deployed, is still a controlled revision.

"Approval before deployment" is not asserted as a universal rule here: what authorization a deployment requires is a matter of the applicable change-control policy, which Arcogine does not own. The invariant this ADR does assert is narrower: **a deployment must be attributable to the authorization the applicable change-control policy required for it** — which may be an explicit approval record, a standing authorization, a pre-approved standard-change classification, an emergency-change justification, or an automated policy decision, depending on what that policy calls for. An `ApprovalRecord` is one authorization mechanism a policy may require, not a universal lifecycle gate every deployment must pass through.

A controlled-revision lifecycle (persistent repository, lineage, approval state, deployment tracking) is deferred capability, to be built only when a concrete requirement (a persistent repository, an external change-management integration, an approval/deployment workflow, an audit requirement, or branching/lineage needs) makes it necessary.
