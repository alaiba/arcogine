# ADR-0005: Explicit-eligibility deterministic dispatch policy
...
The deterministic dispatch capability requires that a single operation step be executable by more than one equivalent installed resource, with deterministic resource selection, while keeping resource capability, operational status, and queue state distinct.

Before this decision, `OperationStepDefinition.eligibleResources` was already modeled as `Set<MachineId>` in anticipation of multi-resource dispatch, but three things collapsed it back to a single machine: `FactoryModelValidator` rejected any step naming more than one eligible resource ("not yet supported by the runtime, this milestone"); the runtime routing type `RoutingStep` had only one `MachineId machineId` field; and `FactoryRuntimeAssembler` discarded every eligible resource but the first (`eligibleResources().iterator().next()`). `FactoryHandler` never compared or ranked candidate machines, because there was never more than one to choose from.

A repository-grounded audit preceding this ADR confirmed the model layer already had the shape deterministic multi-resource dispatch needs; only the validator restriction and the runtime's single-machine routing representation were blocking it. No capability taxonomy, resource pool/work-center type, or generalized scheduler exists or is required by accepted architecture to satisfy this capability's acceptance criteria.

## Decision

The first implementation is classified **A: preserve existing model eligibility into runtime, plus a deterministic selector** — not a new capability abstraction (classification C) and not a canonical-model extension (classification B), since the model already carries what is needed.
...
Model-side eligibility (`OperationStepDefinition.eligibleResources`), runtime operational status (`MachineState`), and runtime queue state (`Machine`'s per-machine `ArrayDeque`) remain three distinct types; this decision does not merge them.
...
- This tie-break policy is now a recorded architectural decision rather than planning-document prose; a future implementation that adds load-aware ranking (e.g. projected completion time) should update or supersede this ADR rather than silently reinterpreting the policy.
