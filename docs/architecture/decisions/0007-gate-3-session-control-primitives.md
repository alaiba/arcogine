...
CommandResult.Faulted<T>(SimError error, FactoryModelVersion modelVersion, List<Event> scheduledEvents)
```
...
`Faulted` exists as a third outcome, distinct from `Rejected`, precisely because it does not carry `Rejected`'s zero-mutation guarantee: it means the command's own preconditions passed, but the runtime then failed while carrying it out, after some mutation and/or event scheduling may already have happened (see "`setMachineAvailability` reports `Faulted`..." below). Collapsing it into `Rejected` would misrepresent that mutation may have occurred; leaving it unhandled would mean the command sometimes returns no result at all. Neither satisfies §7.2's "always returns a definite result."
