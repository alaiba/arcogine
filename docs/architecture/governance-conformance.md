...
> **Authority:** The [controlled revision](controlled-revisions.md) and [Governance evidence](governance-evidence.md) contracts are adopted; the broader capability described here remains proposed. This document does not claim current compliance, audit, or certification capability  
> **Related:** [Product Charter](../product/charter.md), [Architecture Overview](overview.md), [Factory Design Architecture](factory-design.md), [Operational Execution and Digital Twin Architecture](operational-execution-digital-twin.md), [canonical model boundary](factory-design.md#4-canonical-model-boundary), [Factory publication identity contract](factory-design.md#11-publication-identity-and-provenance), [Factory model specification](factory-model.md), [controlled revision contract](controlled-revisions.md), [Operational continuity contract](operational-continuity.md), [Determinism Contract](overview.md#determinism-contract), [Governance evidence contract](governance-evidence.md), [Standards Alignment](standards-alignment.md), [Governance and Conformance Capability Plan](../planning/governance-conformance-capability.md)
...
The [Factory publication identity contract](factory-design.md#11-publication-identity-and-provenance), [Factory model specification](factory-model.md), and the [controlled revision contract](controlled-revisions.md) establish two identities that governance must not collapse:
...
The evidence contract is fixed by the [Governance evidence contract](governance-evidence.md). This section summarizes it and the rest of this document does not extend it.
...
**Intrinsic provenance versus use target.** `Evidence` carries whatever provenance is intrinsic to how it was produced; `EvidenceUse` carries the target it was later used against. A raw external observation is independent of Arcogine model/revision identity at ingestion unless its source intrinsically owns such provenance, and correspondence to an Arcogine subject is a later use or reconciliation determination that is never fabricated at ingestion. An Arcogine-derived analytical or verification result may, and where its producer's contract establishes it must, retain producer-intrinsic `ModelFingerprint`, controlled revision, run/result identity, explicit result-affecting inputs, analytical definition/version, and any exact producer-definition reference that its owning contract actually defines. The current Engine exposes no placeholder definition identifier; missing exact Engine provenance remains missing rather than being inferred or stamped on. The use target may differ from the source model — a `baseline` result used as a comparator for `changed` is an explicit use role, not proof about `changed`. One `Evidence` item may be referenced by many `EvidenceUse` records across multiple model versions and evaluations, as long as each use's scope/applicability determination independently holds; equal fingerprints or a rollback never copy one use's applicability to another target.
...
current factory-model:sha256:<digest> content fingerprint
controlled-revision history in a disposable proving store
exact factory semantic-artifact reconstruction within that store
runtime instantiation from a published model
runtime/result exact-definition provenance intentionally not synthesized
```

The Factory model specification and its implementation establish the current `factory-model:sha256:<digest>` content fingerprint; it is deterministic for one definition but does not identify the exact definition revision that produced it. The controlled revision contract establishes controlled revision identity/lineage, and the Governance identity/history mechanics are implemented: controlled-revision identity and lineage supplies the controlled-revision identity/value contracts in `:types` and `:governance`, while controlled-revision persistence and historical resolution supplies the acceptance/repository boundary, repository-level parent integrity, restart-surviving storage, and exact revision-to-semantic-artifact resolution. The current adapter is a filesystem-backed, explicitly disposable proving store whose factory artifacts reuse the current Factory canonical bytes and whose separate definition-build binding prevents cross-build reinterpretation; no retained, commitment-bearing revision authority has been declared. Governance semantic ChangeSet/impact capability is implemented for its initial slice, as is the generic requirements/assertions capability and deterministic conformance evaluation/findings capability. The headless evidence capability is now implemented additively: source-qualified immutable `EvidenceReference` values with bounded in-memory non-rebinding resolution, contextual `EvidenceUse` values with explicit target/role/applicability, explicit evidence composition, and accepted immutable `EvaluationOccurrence` values with exact captured definitions and an in-memory acceptance boundary. This proves structural evidence plus explicitly attributed analytical and external fixtures; it does not claim Engine/Operational producer integration, durable evidence storage, durable definition storage, or durable evaluation-history persistence. Authorization, deployment, and framework-mapping capabilities remain later work. The evidence/evidence-use semantic contract remains the authority for reference equality/non-rebinding, producer-intrinsic versus use-target provenance, the three applicability/outcome determinations, exact definition resolution, and the point-identity limit.

The Governance dependency is now:

```text
Factory canonical-content fingerprint contract                            complete
    ↓
...
- has a complete generic conformance engine or durable evidence history today (the initial evidence slice is headless and bounded, with no authorization, deployment, or framework-mapping backing it);
...
## 16. Architecture reconciliation triggers

The [Factory publication identity contract](factory-design.md#11-publication-identity-and-provenance) fixes the semantic-identity versus controlled-revision distinction and the external change-control boundary. The [Factory model specification](factory-model.md) owns the current canonical-content fingerprint definition. The [controlled revision contract](controlled-revisions.md) fixes controlled revision identity, current lineage cardinality, rollback semantics, immutable recording provenance, and the persistence boundary. The [Governance evidence contract](governance-evidence.md) fixes evidence reference identity, equality and non-rebinding, evidence versus use, producer-intrinsic versus use-target provenance, applicability versus requirement scope versus outcome, evaluation-occurrence identity, exact definition resolution, and the point-identity limit, while deferring representation, storage, and policy mechanisms.
...
- evidence-reference representation, evaluation-occurrence acceptance/persistence, or durable requirement-definition resolution mechanisms beyond the semantic obligations the Governance evidence contract fixes;
...
Do not add framework-specific architecture merely to record content mappings when the generic governance architecture is unchanged.
