...
| `Job` order-side fields | Product, quantity, agreed unit price, and creation time | Job Order / production request plus commercial facts | Partial | Refactor | Mixed with mutable execution state in the same object |
| `Job` execution-side fields | Status, current step, current machine, and completion time | Job execution state / Work Performance precursor | Partial | Refactor | Request, execution, and result are collapsed |
...
| `OrderCreation` event | Accepted work entering the simulation with product, quantity, and agreed price | Job-order release / request fact | Approximate | Alias | There is no separate first-class production-order aggregate |
...
| `OrderCompleted` | Operational fact that one job completed its routing | Job response / production performance fact | Partial | Alias | `Job` is still one-to-one with an accepted order |
...
Arcogine currently receives work through `OrderCreation` events generated by the demand model. A `Job` immediately becomes both the record of requested work and the mutable execution object.

For explicit production contracts and external consumers, Arcogine should separate immutable order intent from execution state:
...
This compression is reasonable for the current compact simulation. It becomes a refactoring target only when a concrete feature needs independent order intent, multiple work items, plan versions, or structured performance records. The factory-design game supplies that concrete trigger.
