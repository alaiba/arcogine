...
### Semantic-contract support status

[ADR-0017](decisions/0017-semantic-contract-maturity-and-support-promotion.md) makes durability a scoped promise: every semantic contract is explicitly **proving** or **promoted**, silence is never durability, and a promotion is an explicit record. This table is the maintained compact support statement for each current contract; the authority named in each row owns the promise.

| Contract | State | Exact definition / identity | Historical decoding / resolution | Execution support | Outward compatibility | Retention / retirement |
|---|---|---|---|---|---|---|
| `factory-model:v1` | **promoted** (grandfathered; ADR-0017 §9) | ADR-0006 grammar and golden vectors; identifier never reused | permanent verifier/decoder retention and exact historical resolution (ADR-0017 §8 rule 8) | full execution of V1 models under non-spatial semantics (§8 rule 9) | none promised; cross-language reproducibility is promised by ADR-0006 but not yet independently demonstrated | permanent; no automatic V1-to-V2 lift (§8 rule 10) |
| `factory-model:v2` | **proving**; not released, not promoted | [Factory Model v2 Canonicalization](factory-model-v2.md) §10; no fingerprint ever produced | none; no verifier registered, authority rejects V2 artifacts | none; no runtime executes V2 | none | exit condition in V2 §10.3: promotion after Factory composition reconciliation and golden vectors, or retirement/supersession |
| `engine-semantics:v1` | attribution and definition **promoted**; full specification **proving toward release** | [Engine Semantics v1](engine-semantics-v1.md) §15; exercised sections frozen | identifier, specification, and fixtures resolve every attributed run (ADR-0015 decisions 14, 16) | current runtime executes it for V1 models; permanent re-execution not promised | none; identity exposed on `FactoryRuntime` only | spec and fixtures survive retirement; retirement removes executability only |
| legacy `contentHash()` | **proving**, ephemeral in-process custody (ADR-0017 §10) | Java-derived hash; never reinterpreted as a `factory-model:v1` digest (ADR-0006) | none; no retained store holds a bare content hash | in-process `IntegratedHandler`/`SimResult` provenance only | none; not exposed on any API/CLI/web surface | retired when runtime/result provenance migrates to `ModelFingerprint` in outward-consumer convergence |
| supported runtime contract / HTTP | current authority (ADR-0011); outward compatibility deliberately not promoted (ADR-0012) | `RuntimeObservation`/`RuntimeEventEnvelope` | not applicable | current | none promised | not applicable |
| Governance evidence (ADR-0016), durable operational identity (ADR-0013) | accepted logical invariants; representations deferred, no format or storage support promised | ADR text | none implemented | none implemented | none | not applicable |
