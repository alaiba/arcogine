...
The project Repomix is the reviewer's repository-content baseline. Establish exact target `T` with the protocol in `docs/development/repository-snapshot.md`: use the snapshot directly when identical; otherwise use it for unaffected paths and immutable live `T` content for every affected path when a complete descendant delta is available. Repository-wide searches must reconcile snapshot matches with affected target paths. Do not spend connector calls refetching unaffected static content.
...
A snapshot may be older than live `main`; age alone is not a failure. Missing/malformed provenance or inability to establish exact `T`, ancestry, and a complete usable delta makes the review `INCOMPLETE`; refresh the snapshot rather than claiming completeness from an ambiguous corpus.
