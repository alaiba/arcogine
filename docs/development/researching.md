...
When a workspace carrying handed-off evidence needs to incorporate a newer `main`, preserve those exact evidence commits. Do not rebase, force-push, or otherwise rewrite handed-off artifact history merely to satisfy branch freshness. Prefer the native history-preserving merge-style Update branch operation; when that endpoint is unavailable, the repository's narrowly mechanical Git-data fallback is permitted only if it constructs an exact merge tree from complete, non-truncated recursive tree snapshots and advances the branch with `force=false`. A truncated or completeness-unknown tree must be recursively expanded or rejected. In either case, verify the workspace is current with its base and that every handed-off SHA remains reachable. If the tree, identity, ancestry, or mutation guard cannot be proven, stop and return the work to the implementation/author. The invariant is immutable artifact identity, not linear branch history, and the non-forced fallback is not equivalent to exact-head CAS.
