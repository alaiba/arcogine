...
- use repository-scoped GitHub Git-data operations and publish `H -> M` with a non-forced ref update (`force=false`). Do not use a local `gh` prerequisite, rebase, force push, lease, or separate compare-and-swap protocol;
