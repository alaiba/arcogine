...
When a change compresses a document or removes historical narration, check that every rule it carries forward still states its real condition and basis. Replacing a removed rationale with a generic rule whose factual premise is false — for example, asserting that a definition is fixed because records were attributed to it when none were — does not preserve the rule; it creates standing ambiguity about whether and why the rule binds. Either state the real basis or change the rule deliberately.

An explicit owner decision to change architecture is a decision input, not a research conclusion awaiting acceptance. A PR implementing one is reviewed for faithful, complete and truthful implementation and reconciliation of that decision — not for whether the decision is permitted by the rules it replaces — and it must not be presented as adoption of a research result that was not accepted. Research conclusions that a PR does promote still need the independent review the [research operating model](researching.md) requires, and ordinary independent PR review and the owner's manual merge apply to both.
