...
- **Node.js:** the frontend support contract is **`^22.22.2 || ^24.15.0 || >=26.0.0`**, declared in `product/interfaces/web/package.json`. This range reflects the current locked dependency graph; it is not equivalent to a simple `22+`. CI pins **22.22.2** to exercise the lowest supported Node 22 release, while the preferred devcontainer currently uses Node 24.
