...
- **Java:** JDK 21 is the supported build floor. Every Java compile task uses `--release 21`, which fixes the accepted language level, Java API surface, and generated bytecode to Java 21 even when the compiler itself comes from a newer supported JDK. CI runs on Temurin 21 to prove the actual floor; the preferred devcontainer currently uses JDK 25.
- **Gradle:** always use the wrapper at `product/gradlew`, currently Gradle **9.7.0**. No system Gradle is required.
- **Node.js:** the frontend support contract is **`^22.22.2 || ^24.15.0 || ^26.0.0`**, declared in `product/interfaces/web/package.json`. This range reflects the current locked dependency graph, constrained to the Node majors Arcogine deliberately supports; it is not equivalent to a simple `22+`. CI pins **22.22.2** to exercise the lowest supported Node 22 release, while the preferred devcontainer currently uses Node 24.
...
- The `arcogine_gradle_cache` named volume caches the downloaded Gradle distribution and dependencies across runs — without it, each fresh container re-downloads `gradle-9.7.0-bin.zip`. Neither the volume nor the `arcogine-build` container survive a move to a different machine; recreate them there with the commands above.
