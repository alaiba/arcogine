...
> **Status:** Active/partial; current Factory Model v1 capability and the V2 optional-record shape/validation proving slice are landed; the Engine applicability question is concluded and reconciled ([Engine Semantics v2](../architecture/engine-semantics-v2.md) applies to `factory-model:v2`), so V2 canonical identity is now blocked only on its own implementation, and cross-policy historical resolution waits for a real transition
>
...
**Status:** Not started; blocked only on its own implementation. The Factory composition prerequisite is satisfied: the grammar to implement is the reconciled [Factory Model v2](../architecture/factory-model-v2.md). The [Engine applicability question](../research/investigations/engine-evolution.md#concluded--engine-applicability-to-optional-record-factory-policies) is concluded and reconciled: [Engine Semantics v2](../architecture/engine-semantics-v2.md) is the identity that executes this grammar, in both its spatial-present and spatial-absent forms. Its measured implementation/support cost remains the Factory-composition reopening trigger to weigh before release.
