...
A slice that makes **layout** behaviorally consequential has an additional upstream semantic dependency: the READY [Engine-applicability question](../research/investigations/engine-applicability-after-transfer-boundary.md), its reconciliation, and the spatial runtime work selected afterward. The [current transfer boundary](../architecture/transfer-applicability.md) is already reconciled.
