...
7. **Gate 4 — stable runtime observations and ordered authoritative runtime events. Active.** G4-A is complete: `FactoryRuntime.observe()` supplies an immutable, headless current-state projection with opaque per-runtime `RunId`, durable `ModelFingerprint`, simulated time, explicit active/quiescent run state, the reserved zero supported-event cursor, ordered resource/order/W1-child/pending-work state, and existing authoritative performance aggregates. It does not expose or rebrand internal scheduler events. G4-B supported runtime events and G4-C headless closure remain outstanding; see [ADR-0011](../architecture/decisions/0011-runtime-observation-and-event-contract.md) and [Gate 4 Runtime Observation and Event Delivery](gate-4-runtime-observation-event-delivery.md).
