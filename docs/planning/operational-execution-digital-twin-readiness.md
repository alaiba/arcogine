...
ADR-0013 now proposes the O1 representation and evolution rules that were previously open: UUIDv4 identity with strict canonical parsing, `PRODUCTION`/`STAGING`/`SIMULATION` taxonomy, explicit unknown-kind failure at the semantic boundary, decentralized establishment through the O1 boundary, permanent ID-to-kind binding, and explicit versioned migration handling for public/persisted changes.
...
If ADR-0013 is Accepted as proposed, `ExecutionContextKind` contains `PRODUCTION`, `STAGING`, and `SIMULATION`; `ExecutionContextId` is opaque UUIDv4; and `ExecutionContext` is an immutable pair of ID and kind with permanent ID-to-kind binding.
...
- one ID is permanently bound to one kind; consequence-class change creates a new ID;
...
- the same ID cannot be rebound to a different kind where the mismatch is observable;
...
6. an immutable `ExecutionContext` containing only ID and kind;
7. the dedicated `:operational` ownership/dependency direction for the first implementation;
8. versioned projection/migration rules under ADR-0012 and explicit failure for unsupported semantic kinds;
9. durable restart identity semantics without a context registry;
10. preservation of raw external-observation independence and all neighboring identity boundaries.
