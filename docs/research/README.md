...
| When should Arcogine evolve beyond its current deterministic dispatch policy? | Engine | Medium | **CONCLUDED** | [Engine evolution research](engine-evolution.md) | **Do not add a general policy menu/scheduler now**; preserve current behavior as the implementation baseline, with the narrower pre-release questions below and same-semantics performance work separated | 2026-09-11 |
| Before `engine-semantics:v1` release, should local queue admission remain at most one job per cascade trigger or adopt a bounded non-idling rule when one trigger exposes several free slots? | Engine | **Critical-path** | **READY** | [Engine evolution research](engine-evolution.md) | Explicit pre-release Engine-semantics decision, then conformance/implementation if the rule changes | 2026-09-11 |
| Before `engine-semantics:v1` release, should flexible-resource ranking keep exact `combinedQueueDepth` when shared pending jobs overlap several candidate machines? | Engine | **Critical-path** | **READY** | [Engine evolution research](engine-evolution.md) | Explicit pre-release Engine-semantics decision, then conformance/implementation if the ranking changes | 2026-09-11 |
