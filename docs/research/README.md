...
Durable investigation briefs and deliberately retained or reconciled research-history artifacts belong under [`investigations/`](investigations/), rather than beside the maintained registers and reusable templates. Temporary decision-quality reports, report revisions, adversarial reviews, checkpoints, and handoff material belong under `workspace/research/` while they are serving evidence custody, and must be removed from the final merge candidate. Exact workspace SHA+path coordinates identify those artifacts only during active custody; maintained state after retirement must not depend on a Git host retaining the corresponding commits. Any result that must survive is reconciled into its durable destination first, and any artifact whose exact future readability matters must be deliberately promoted to a retention surface with a real persistence contract; otherwise the temporary file still leaves the final tree. These artifacts are research evidence or investigation framing, not accepted product/architecture semantics merely because they exist.
