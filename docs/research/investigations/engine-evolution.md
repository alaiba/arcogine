...
The fixed [Engine v1 definition](../../architecture/engine-semantics-v1.md) includes spatial rules specified when Factory V2's spatial facts were mandatory. The [Factory composition result](factory-model-semantic-composition.md) made that record optional, while keeping Factory validity distinct from Engine applicability. The former investigation of both V2 forms is retained as [SUPERSEDED research evidence](engine-applicability-optional-record-factory-policies.md): it assumed the absent case had no transfer lifecycle. The [current transfer boundary](../../architecture/transfer-applicability.md) has since selected that meaning for admitted V2-absent content with explicit qualifications. The [successor applicability brief](engine-applicability-after-transfer-boundary.md) is READY to investigate exact Engine identity and support. The [research register](../research-register.md) owns both lifecycle states.

V2 publication and spatial runtime activation remain blocked on the successor applicability investigation and reconciliation; see [Spatial Runtime Consequences](../../planning/spatial-runtime-consequences.md).
