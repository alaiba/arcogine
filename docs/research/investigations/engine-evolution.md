...
> **Lifecycle:** See the maintained research register; this artifact contains concluded dispatch history, concluded first-release dispatch decisions, context and links for the superseded Engine-applicability investigations, and other CANDIDATE follow-up questions. The retained rules were called `engine-semantics:v1` when these decisions were made; after the provisional semantic-contract reset they are the current, work-in-progress Engine rules
>
...
> **Authority:** Research only; current Engine semantics are defined by adopted architecture and pinned by executable evidence, and research here does not change them

## Purpose

The Engine implementation must not silently evolve result-affecting policy. This document holds candidate extensions and bounded follow-up questions until evidence and an explicit architecture change justify a different implementation contract.

The [Engine interpretation](../../architecture/engine-semantics.md) is normative even though its implementation is partial, and it is work in progress under the [Determinism Contract](../../architecture/overview.md#determinism-contract) and the [semantic evolution and support rules](../../architecture/overview.md#semantic-evolution-and-support): an intentional change that can alter outcomes for identical explicit inputs is an explicit change to the Engine specification and its conformance fixtures. Research may decide whether the current rules are acceptable or whether evidence justifies such a change. Neither outcome permits a silent implementation tweak.
...
The current one-local-job-per-trigger rule and `combinedQueueDepth` ranking are normative Engine rules and production currently conforms to them. Historical review found that the one-job rule was inherited implementation behavior later captured by the normative wording, not evidence of an originally evaluated scheduling trade-off.

That history is material motivation to re-evaluate the rule, but it does not make the rule an accident to change quietly:

- retaining either rule leaves the Engine specification unchanged and is pinned by its conformance fixtures;
- selecting an outcome-changing alternative is evidence for an explicit Engine specification change with updated fixtures; it is not an ordinary bug fix or an implementation-only edit;
- implementation must not adopt an alternative until that specification change exists.

The two first-release dispatch questions below have now completed standard investigation and independent adversarial review. Both reviews ended **ACCEPT WITH QUALIFICATIONS**, both retain the existing rule, and their durable consequences are reconciled jointly below because the ranking rule's exceptional magnitude-sensitive regime is created by the retained local-admission behavior at recovery.
...
The two bounded first-release questions were investigated separately so that one experiment could hold the other rule fixed. Their reviewed conclusions converge on one release decision: **retain the current rules unchanged for both local admission and shared flexible-backlog ranking**.

This is a conservative compatibility decision, not a claim that either rule is globally optimal or historically well-motivated. No accepted Arcogine objective selects a better result-affecting alternative, while each tested alternative has valid workloads where it regresses another outcome. Because both retained rules are already normative Engine semantics, this reconciliation requires no specification change.
...
The executable handoff belongs to the Engine-conformance slice in [Spatial Runtime Consequences](../../planning/spatial-runtime-consequences.md). That planning surface owns characterization fixtures and the already-identified arithmetic corrections; this research document owns only the decision, qualifications, and reopening triggers.

## CONCLUDED — local admission semantics before first Engine release

### Question

Should the first supported Engine release retain the current at-most-one-local-job-per-cascade-trigger rule, or does the evidence justify a different, precisely bounded rule that admits more local work when one trigger exposes several immediately usable concurrency slots?

### Decision at stake

Whether Arcogine keeps the current local-admission rule for the first supported release or adopts an alternative through an explicit Engine specification change before changing implementation behavior.

### Reconciled conclusion

Retain the existing rule: after the established continuation/next-step processing, a cascade admits **at most one** job from the affected machine's own FIFO queue before running the existing shared-pending fixpoint. No different admission rule is justified for the first supported release, and no silent implementation change is authorized.
...
- **Inherited asymmetry, not a general cascade invariant.** Only the local-queue stage is budgeted at one. Initial order admission and the shared-pending fixpoint already fill available capacity. Durable wording must not describe the cascade as generally "trigger-budgeted" or globally non-work-conserving.
...
- **Retention is provisional, not optimality.** The supported statement is "no current evidence justifies changing the rule," not "one-local-job admission is intrinsically correct." Empirical recovery behavior from a supported consumer can reopen the question if the rule materially misrepresents the plant being modelled.
...
- **Retain the current rule:** one local queued job at most per cascade trigger, followed by the existing shared-pending fixpoint.
- **Recovery-specific fill:** after a genuine offline -> online transition, admit local FIFO work up to currently usable capacity before the shared-pending stage; ordinary completion retains one-local-job behavior because it normally releases one slot. Selecting this outcome would be an explicit Engine specification change.
- **Trigger-independent bounded non-idling:** whenever the local-queue stage runs, admit local FIFO work until no immediately usable slot remains, then run the existing shared-pending fixpoint. Selecting this outcome would be an explicit Engine specification change.
...
- one offline concurrency-4 machine with eight queued 10-tick single-eligible jobs: the retained rule completes after 80 ticks from recovery, while recovery fill would complete after 20;
- mixed recovery with M1 concurrency 4, M2 offline, two local M1-only 10-tick jobs and four shared `{M1,M2}` 3-tick jobs: the retained local stage admits one and the shared fixpoint fills remaining capacity, proving local-before-shared order does not imply exclusive local priority;
- the routed regression with order completions `[14,29,29]` and makespan 29 under the retained rule versus `[14,40,21]` and makespan 40 under fill;
- the reviewed trade-off with makespan/mean-order completion `93 / 69.75` under the retained rule versus `91 / 71.25` under fill;
...
The retained rule now has an explicit bounded rationale independent of utilization, exact ordering is already normative, discriminating cases have been reproduced independently, and the high-risk conclusion received adversarial review with **ACCEPT WITH QUALIFICATIONS**. The durable consequence is no semantics change: preserve the rule and pin it executably, subject to the qualifications above.

## CONCLUDED — shared flexible-backlog ranking semantics before first Engine release

### Question

Should the first supported Engine release retain the exact `combinedQueueDepth` — local physical queue depth plus every compatible shared-pending entry — when one unbound flexible job contributes demand to several candidate machines simultaneously, or does the evidence justify another ranking term?

### Decision at stake

Whether Arcogine keeps the current resource-selection ranking or adopts a different queue-depth/ranking term through an explicit Engine specification change while preserving eligibility, online preference, immediate-acceptance ranking, unbound shared waiting/reselection, and final `MachineId` tie-breaking.

### Reconciled conclusion

Retain the exact `combinedQueueDepth` ranking unchanged. No tested alternative has a better-supported semantic invariant under Arcogine's current objective boundary, so this question justifies no Engine specification change and authorizes no silent implementation change.

This conclusion is not a claim that `combinedQueueDepth` is a principled physical-load estimate or an optimal dispatch rule. The shared contribution is best understood conditionally: when competing accepting machines have empty local queues, the non-zero shared contribution acts as a handover signal that tends to leave the just-freed machine available to compatible unbound work. Outside that sub-domain — most importantly the recovery state created by the retained local admission — the magnitude of the term can affect the choice.
...
- **The candidate space is not universally two-valued.** Positive reweightings collapse to the current ranking only on the sub-domain where competing accepting candidates do not themselves carry residual local queues. A recovered concurrency>1 machine can violate that condition under the retained local admission, and positive weights can then produce distinct assignments.
...
- **Information horizon must be stated precisely.** The mirror evidence proves that no function of the facts the **current comparator consults** can separate both outcomes. It does not prove that every fact available at that instant is unavailable to a future different rule. Shared-entry duration/routing position, the ranked job's current-step duration/remaining authored route, free capacity, eligible-set structure, backlog order, `busyTicks`, and current time already exist but are unused. Remaining processing time of an active job is genuinely unavailable today because the runtime exposes neither per-step start time nor scheduler enumeration.
...
- **Retain the exact `combinedQueueDepth`** as currently specified.
- **Local physical queue depth only**, leaving shared work unbound and reconsidered at dispatch time rather than projecting it into every candidate's depth. Selecting this outcome would be an explicit Engine specification change.
...
1. **Canonical overlap discriminator**
   ...
   with all machines unary. The current ranking produces `[106,6,105]`, mean 72.33, makespan 106. Local-depth-only produces `[7,6,106]`, mean 39.67, makespan 106. The waiting shared job must remain genuinely unbound and be reselected later.

2. **One-variable mirror.** Use the same construction but change only `O3`'s duration from 100 to 1. The decisive current-comparator state is unchanged, but the current mean is 6.33 versus 6.67 under local-depth-only. Together with the canonical case, this proves that the canonical improvement does not select a replacement policy over the incumbent comparator's fact projection.

3. **Scarce-machine protection.** `O1: M1:5 -> {M1,M2}:60`; `O2: M3:500`; `O3,O4: {M1,M3}:20`. The current ranking completes `[65,500,25,45]`, mean 158.75; local-depth-only completes `[65,500,85,105]`, mean 188.75.

4. **Makespan non-neutrality.** Use the canonical routes with concurrency 2 on M1 and M2 and quantities `2 / 1 / 3`. The current ranking yields `[106,6,106]`, mean 72.67, makespan 106; local-depth-only yields `[7,6,107]`, mean 40.00, makespan 107.

5. **Overlap multiplicity without early binding.** Project one 100-tick shared job onto `{M1,M3,M4,M5}` while M3/M4/M5 are each held for 500 ticks. Both the current ranking and local-depth-only must preserve unbound waiting/reselection even though the shared contribution is counted against several candidates.

6. **Recovery magnitude-sensitive corner.** Construct a ranking call with a recovered concurrency-4 machine still accepting with local depth 3 under the retained one-local-job recovery stage, competing with a just-freed unary machine that has two compatible pending shared entries. Exact current keys are 3 versus 2 and select the unary machine; a positive weight of 2 produces 3 versus 4 and selects the recovered machine. This pins the qualification that positive-weight terms do not universally collapse to the current ranking and couples the ranking case to retained local admission.
...
## Engine applicability to optional-record Factory content

The [Engine specification](../../architecture/engine-semantics.md) includes spatial rules first specified when spatial facts were mandatory. The [Factory composition result](factory-model-semantic-composition.md) made the spatial record optional, while keeping Factory validity distinct from Engine applicability. The former investigation of both forms is retained as [SUPERSEDED research evidence](engine-applicability-optional-record-factory-policies.md): it assumed the absent case had no transfer lifecycle. The [current transfer boundary](../../architecture/transfer-applicability.md) has since selected that meaning with explicit qualifications. The [successor brief](engine-applicability-after-transfer-boundary.md) about exact Engine identities was superseded by the provisional semantic-contract reset, which made the Engine interpretation work in progress. The [research register](../research-register.md) owns both lifecycle states.

The current Engine executes production records only and refuses present spatial content; making spatial content executable is ordinary delivery in [Spatial Runtime Consequences](../../planning/spatial-runtime-consequences.md).
...
This concern is ready for implementation planning only under an exact-semantics constraint. Optimize the ranking semantics of the current Engine interpretation without changing selected resource assignments, local/shared waiting order, shared-backlog reselection behavior, supported events/observations, deterministic replay, or exact ranking arithmetic. If an optimization requires different results, stop and return to research rather than hiding policy evolution inside a performance change.
