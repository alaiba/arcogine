...
A narrower upstream boundary is now explicitly open in [Transfer semantics boundary research](transfer-semantics.md): whether inter-resource transfer lifecycle itself is independent of spatial layout, and only then whether any positive non-spatial transfer timing contract is justified. That question may reopen the still-unreleased V2 grammar before attribution; it is distinct from the richer spatial/material-flow capabilities below.
