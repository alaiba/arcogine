...
>
> Copy the structure below into the new report file under `workspace/research/investigations/`; delete this note and any section headers that do not apply to the question at hand. Durable research briefs and reconciled conclusions remain under `docs/research/`.
...
For a write-up still awaiting independent review, distinguish the author's own self-challenge from a later independent pass — do not present a self-administered check with the weight of independent review. Once an independent adversarial review exists, keep this reviewed report revision unchanged: the review artifact records this report's exact commit SHA and its disposition (`ACCEPT`, `ACCEPT WITH QUALIFICATIONS`, `MORE EVIDENCE REQUIRED`, or `REOPEN` — see `docs/development/researching.md` §9), and later reconciliation must resolve the exact report/review pair while active custody continues. Those coordinates are not a post-retirement preservation mechanism. If the report itself is revised after review, that new commit is a distinct report revision and, where independent review is required for promotion, must receive its own applicable adversarial review before its conclusions are promoted.
