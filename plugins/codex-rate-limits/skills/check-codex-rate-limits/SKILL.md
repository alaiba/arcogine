...
description: Report the current Codex ChatGPT subscription rate-limit windows, especially the 5h and Weekly limits, using the local Codex app-server instead of guessing.
---

# Check Codex Rate Limits

Use this skill when the user asks about their current Codex ChatGPT subscription room, especially requests involving the `5h` or `Weekly` windows.
...
- If `account/rateLimits/read` fails, say that this local Codex build or account does not expose ChatGPT subscription rate limits and do not guess.
