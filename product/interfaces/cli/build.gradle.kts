...
// Stages the executable jar into the repository-wide dist/ contract
// (dist/api/arcogine.jar), consumed by infra/docker/api.Dockerfile.
tasks.register<Copy>("stageDist") {
    dependsOn(tasks.bootJar)
    from(tasks.bootJar.flatMap { it.archiveFile })
    into(rootProject.layout.projectDirectory.dir("../dist/api"))
}
